---
title: "0719-Lecture: Machine learning"
...
NOTE: it makes each tree more independent from each other to reduce correlation. if we force the trees to consider random selections of variables, each tree could be adding new independent information that other trees don't capture
...
# Purpose: Demo random forest and gradient boosted trees
...
- complexity of the trees (depth, number of observations in nodes) NOTE: this matters more in boosting than in random forests; tend to use many small nodes, high depth captures more interaction
...
NOTE: boosting is sequential; it is an additive model