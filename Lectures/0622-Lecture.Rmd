---
title: "0622-Lecture:Supervised Learning"
...
```{r nba-table}
table(nba_mclust$classification)
...
nba_filtered_stats %>%
     ...
     ggplot(aes(x =  x3pa, y = trb, color = gmm_class, size = gmm_uncertainty)) +
     geom_point(alpha = 0.5) +
     scale_size(guide = FALSE) + # each scale has a guide
     # could also do:
     guides(size = FALSE) +
     theme_minimal() +
...
```{r}
# probability distribution for an observation
nba_mclust$z
...
Note: if points were perfectly separated, all values would appear at 0 and 1 in these histograms
...
nba_filtered_stats %>%
  ...
  slice(1:5) %>%
  ggplot(aes(y = gmm_uncertainty, 
             x = reorder(player, #<<
                         gmm_uncertainty))) +#<<
  geom_point() +
  coord_flip() + 
  theme_bw() +
  facet_wrap(~ gmm_class, 
             scales = 'free_y', nrow = 3) # adjusts categorical levels in each facet plot

```

End continued material from 06-21.
