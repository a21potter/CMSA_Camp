---
title: "0720-Lecture: Advanced topics"
...
__Expected Points:__ Measure the value of play in terms of $\mathbb{1}[\text{points of next scoring play}]$
...
init_loso_cv_preds <- 
  map_dfr(unique(nfl_ep_model_data$season), 
          function(x) {
            ...
            ep_model <- 
              multinom(Next_Score_Half ~ half_seconds_remaining +
                         yardline_100 + down + log_ydstogo +
                         log_ydstogo*down + yardline_100*down, 
...
(loading previously evaluated init_loso_cv_preds for time saving purposes)
...
ep_cv_loso_calibration_results <- init_loso_cv_preds %>%
  ...
  summarize(n_plays = n(), 
            n_scoring_event = length(which(Next_Score_Half == next_score_type)), # comparison gives TRUEs and FALSEs, which() gives only the TRUEs, length() gives how many are TRUE
            bin_actual_prob = n_scoring_event / n_plays,
            bin_se = sqrt((bin_actual_prob * (1 - bin_actual_prob)) / n_plays)) %>% #<<
  ungroup() %>%
  mutate(bin_upper = pmin(bin_actual_prob + 2 * bin_se, 1), #<< remember to bin max value as 1 since its a probability
         bin_lower = pmax(bin_actual_prob - 2 * bin_se, 0)) #<< remember to bin min value as 0 since its a probability
```
...
ep_cv_loso_calibration_results %>%
  ...
  # faceting because making a calibration plot for each score type
...
NOTE: Q are quarterbacks, C are receivers; on the cusp of Bayesian because the average of all quarterbacks and receivers act similar to a prior, however this is still frequentist because the averages are not from prior data, they are calculated from the current data
...
NOTE: Including the random effects of individual players helps solve the violation of independence otherwise
