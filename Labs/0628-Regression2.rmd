...
The coefficients in the model have changed.
...
nba_data_2022 %>%
  ...
  ggplot(aes(x = free_throw_percentage, y = pos_preds, color = position)) +
  geom_point(alpha = 0.5) +
  theme_bw() +
...
Since there is no interaction, the lines are parallel. PG is shifted up, followed by PF, C and SG and SF are very similar on the bottom.
...
```{r}
x_pos_nba_lm_group <- lm(min_per_game ~ position_group + free_throw_percentage, data = nba_data_2022)

summary(x_pos_nba_lm_group)
```

The adjusted $R^2$ value increased in comparison to the previous values.
...
Now the model predictions match the line produced by geom_smooth().
...
Although the adjusted $R^2$ is higher, this is not appropriate. With so many degrees, this is overfitting the field goal percentage.
...
The candidate model 1 performed better because it has the lower MSE.
...
```{r}
n_players_2021 <- nrow(nba_data_2021)
train_i_2021 <- sample(n_players_2021, n_players_2021 / 2, replace = FALSE)
test_i_2021 <- (1:n_players_2021)[-train_i_2021]
nba_train_2021 <- nba_data_2021[train_i_2021,]
nba_test_2021 <- nba_data_2021[train_i_2021,]
```


```{r}
candidate_model_1_2021 <- lm(min_per_game ~ poly(field_goal_percentage, 2) + position +
                          position * poly(field_goal_percentage, 2), 
                        data = nba_train_2021_filter)

#summary(candidate_model_1_2021)

model_1_preds_2021 <- predict(candidate_model_1_2021, newdata = nba_test_2021)

model_1_mse_2021 <- mean((model_1_preds_2021 - nba_test_2021$min_per_game)^2)
```

```{r}
candidate_model_2_2021 <- lm(min_per_game ~ poly(field_goal_percentage, 2) + position, 
                        data = nba_train_2021)

#summary(candidate_model_2_2021)

model_2_preds_2021 <- predict(candidate_model_2_2021, newdata = nba_test_2021)

model_2_mse_2021 <- mean((model_2_preds_2021 - nba_test_2021$min_per_game)^2)
```


The candidate 2 2021 model performs the best on 2021 test data.

```{r}
model_2_preds_2022 <- predict(candidate_model_2_2021, newdata = nba_data_2022)

model_2_mse_2022 <- mean((model_2_preds_2022 - nba_data_2022$min_per_game)^2)
```
